...
function renderIncomePage(){
  ...
  // ── N잡 수입 데이터 집계 (freelance.js와 동일한 atm2_income_ 키 사용) ──
  let njobItems = [];
  try{
    const incKey = `atm2_income_${curY}_${_p(curM+1)}`;
    const raw = localStorage.getItem(incKey);
    if(raw) njobItems = JSON.parse(raw);
  }catch(e){}

  const njobGross = njobItems.reduce((s,it)=>s+(parseInt(it.amount)||0), 0);
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
    ...
    <div onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none';this.querySelector('.acc-arr').textContent=this.nextElementSibling.style.display==='none'?'▼':'▲'"
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="font-size:16px;">💼</span>
        <span style="font-size:13px;font-weight:700;color:var(--orange);">N잡 수입</span>
        <span style="font-size:11px;color:var(--text3);">\${njobItems.length}건</span>
      </div>
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="font-size:14px;font-weight:700;color:var(--orange);">\${njobNet.toLocaleString()}원</span>
        <span class="acc-arr" style="font-size:12px;color:var(--text3);">▼</span>
      </div>
    </div>
    <div style="display:none;padding:14px 16px;">
      \${njobItems.map(it=>{
        ...
        const ICONS={convenience:'🏪',delivery:'🛵',driver:'🚗',freelancer:'💻',shortAlba:'📋',etc:'➕',employee:'🏢'};
        const icon=ICONS[it.jobType]||'💼';
        return \`<div style="display:flex;justify-content:space-between;align-items:center;
                    padding:7px 0;border-bottom:1px solid var(--border);">
          <div>
            <div style="font-size:13px;font-weight:600;color:var(--text);">\${icon} \${it.label||it.jobType||'수입'}</div>
      ...
      <div style="display:flex;justify-content:space-between;padding:8px 0;border-top:1px solid var(--border);margin-top:4px;">
        <span style="font-size:12px;color:var(--text2);">3.3% 공제 후 실수령</span>
        <span style="font-size:14px;font-weight:700;color:var(--green);">\${njobNet.toLocaleString()}원</span>
      </div>
    </div>
  </div>` : ''}
